...
try:
    ...
    # This notebook only needs vector_add for Backgammon.  Keeping this small
    # fallback here lets Tic-Tac-Toe run when games.py is used on its own.
    def vector_add(a, b):
...
def alpha_beta_player(game, state):
    """Choose the optimal legal move using a full alpha-beta search.

    The function matches the player interface used by ``Game.play_game``:
    every player receives ``(game, state)`` and returns one legal move.
    """
    return alpha_beta_search(state, game)


# Backwards-compatible names used by the accompanying teaching notebook.
