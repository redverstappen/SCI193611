        ...
        if not self.is_done():
            ...
            for agent in self.agents:
                if agent.alive:
                    ...
                    actions.append(agent.program(p))
                else:
                    actions.append("")
...
class XYEnvironment(Environment):
    ...
    def is_inbounds(self, location):
        ...
        x, y = location
        return (self.x_start <= x < self.width and self.y_start <= y < self.height)

    def random_location_inbounds(self, exclude=None):
        """Returns a random location that is inbounds (within walls if we have walls)"""
        location = (random.randint(self.x_start, self.width - 1),
                    random.randint(self.y_start, self.height -1))
